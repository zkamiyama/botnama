export const translations = {
  en: {
    ...
    reason_concurrent_limit: ({ limit }) => limit
      ? `You can have up to ${limit} active request(s).`
      : "You hit the concurrent request limit.",
  },
  ja: {
    summary_items: "残",
    pending_items: (n) => `${n}`,
    ...
    rule_concurrent_enable: "同時リクエスト数を制限",
    rule_concurrent_max: "ユーザーごとの最大件数",
    ...
    ng_user_remove_done: ({ user }) => `${user ?? ""} をNGから解除しました`,
    ng_user_clear_done: "NGユーザー一覧をクリアしました",
    ng_user_action_fail: (msg) => `NGユーザーの更新に失敗しました: ${msg}`,
    ...
    reason_site_disabled: ({ siteKey }) => {
      ...
      return `${name} からのリクエストは現在無効化されています`;
    },
    reason_ng_user: ({ user }) => `NGユーザーとして拒否しました: ${user ?? ""}`,
