...
const PROTO_DIR = join(PROJECT_ROOT, "external", "nicolive-comment-protobuf", "proto");
...
export class NDGRClient {
    ...
    private async loadProto(): Promise<void> {
        ...
        try {
            ...
            this.protoRoot.resolvePath = (_origin: string, target: string) => {
                if (target.startsWith('/') || target.startsWith('\\') || target.match(/^[a-zA-Z]:/)) {
                    return target;
                }
                return join(PROTO_DIR, target);
            };

            await this.protoRoot.load([payloadPath], { keepCase: true });
        } catch (error) {
            console.error("Failed to load protobuf definitions:", error);
            throw new Error(
                "Protobuf definitions not found. Make sure to clone n-air-app/nicolive-comment-protobuf to external/nicolive-comment-protobuf",
            );
        }
    }
